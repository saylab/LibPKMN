...
namespace pkmn
{
    namespace conversions
    {
                                              ...
                                              bool is_encrypted)
        {
            ...
            // Block B
            for(size_t i = 0; i < 4; i++) t_pkmn->set_move(database::get_move_name(blocks.blockB.moves[i]), (i+1));
            for(size_t i = 0; i < 4; i++) t_pkmn->set_move_PP(blocks.blockB.move_pps[i], (i+1));
        ...
        void export_gen5_pokemon(team_pokemon::sptr t_pkmn, pkmds::party_pkm* p_pkm)
        {
            ...
            p_pkm->ball = pkmds::Balls::balls(libpkmn_ball_to_game_ball(reverse_ball_dict.at(t_pkmn->get_ball(), Balls::POKE_BALL)));
