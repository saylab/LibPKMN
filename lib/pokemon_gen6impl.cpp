...
namespace pkmn
{
                     ...
                     version)
    {
        ...
        try
        {
            uint16_t pokemon_id = database::get_pokemon_id(_blockA->species, Versions::X);
            _invalid = false;
        }
        catch(...)
        {
    ...
    void pokemon_gen6impl::set_shiny(bool value)
    {
        ...
        uint8_t hid = (_blockA->personality & 0xFFFF0000) >> 16; 
        ...
        uint8_t lid = (_blockA->personality & 0xFFFF);
        ...
        _blockA->personality = ((hid << 16) | lid);
    }
