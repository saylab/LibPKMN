...
int DLL_EXPORT fs_initsave(u8 *nsav);
void DLL_EXPORT fs_setupdateflags();
u32 DLL_EXPORT fs_getsave_mediasize(u8 *part);
u32 DLL_EXPORT fs_get_start(u8 *buf);
u32 DLL_EXPORT fs_get_offset(u8 *buf);

DLL_EXPORT partition_table *fs_part_get_info(u8 *buf, u32 part_no);
DLL_EXPORT u8 *fs_part(u8 *buf, int fs, int datapart, int part_tableno);
DLL_EXPORT u8 *fs_get_savepart(u8 *buf);
DLL_EXPORT u8 *fs_getfilebase();
int DLL_EXPORT fs_verifyupdatehashtree_fsdata(u32 offset, u32 size, u8 *databuf, int filedata, int update);
