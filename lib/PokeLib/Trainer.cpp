...
//		}
    ...
    pid = *((uint16_t*)&sav->BlockA[tOffset[offsetTID][type]]);
    sid = *((uint16_t*)&sav->BlockA[tOffset[offsetSID][type]]);
    isFemale = (sav->BlockA[tOffset[offsetGender][type]] == 0 ? false : true);

	money = *((uint32_t*)&sav->BlockA[tOffset[offsetMoney][type]]);
...
uint32_t Trainer::getTID() const {
    return tid;
}
uint16_t Trainer::getPID() const {
    return pid;
}
uint16_t Trainer::getSID() const {
    return sid;
}
void Trainer::setTID(const uint32_t id) {
    tid = id;
}
void Trainer::setPID(const uint16_t id) {
    pid = id;
}
void Trainer::setSID(const uint16_t id) {
    sid = id;
}
...
//Money Stuff
uint32_t Trainer::getMoney() const {
    return money;
}
void Trainer::setMoney(uint32_t amount) {
    money = amount;
}
