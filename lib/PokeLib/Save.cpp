...
Save::Save(const char* filename) {
    ...
    FILE* save_file;
    #ifdef _MSC_VER
    fopen_s(&save_file, filename, "r+b");
    #else
    save_file = fopen(filename, "rwb");
    #endif
    ...
    else if(size > 0x40000 && size < 0x80000) {
        save_size = 0x40000;
        format = RAW256KiB;
    }
    else
    {
        throw std::runtime_error("File is too small to be a proper save file.");
    }
    data = (uint8_t*)malloc(size);
    int result = fread(data, 1, save_size, save_file);
    if(result != save_size)
    {
        throw std::runtime_error("Problem reading save file.");
    }
    fclose(save_file);
}
