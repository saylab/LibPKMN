...
int fuse_sav_init(u8 *buf, u32 size, u8 *xorpad, u32 xorpad_sz, int argc, char *argv[]) {
    sav_operations.getattr = sav_getattr;
    sav_operations.readdir = sav_readdir;
    sav_operations.open    = sav_open;
    sav_operations.read    = sav_read;
    sav_operations.write   = sav_write;
	...
	xor_3dsfuse(sav_buf, sav_size, out_buf+0x2000, xorpad_buf, xorpad_size);

	if(fs_initsave(sav_buf))
		return 1;

#ifdef _MSC_VER
    return fuse_main(argc, argv, &sav_operations, NULL);
#else
